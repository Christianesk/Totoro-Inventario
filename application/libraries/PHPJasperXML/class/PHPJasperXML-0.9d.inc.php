...
class PHPJasperXML {
    ...
    public function connect($db_host,$db_user,$db_pass,$db_or_dsn_name,$cndriver="mysql") {
        ...
        if($cndriver=="mysql") {
            ...
            if($this->myconn) {
                ...
                return false;
        }elseif($this->left($cndriver,5)=="adodb"){
            $driver=explode(":",$cndriver);
            
            $this->myconn = NewADOConnection($driver[1]);
           $this->myconn = $this->db->Connect($db_host, $db_user, $db_pass, $db_or_dsn_name);
          $this->myconn->SetFetchMode(ADODB_FETCH_ASSOC);
        }
        else {
    ...
    public function load_xml_string($jrxml){
        ...
        // $frameH = '<frame>';
        // $frameT = '</frame>';
        // $jrxml = str_replace($frameH, '', $jrxml);
        // $jrxml = str_replace($frameT, '', $jrxml);
    ...
    if($_REQUEST['forceexceloutput']!=""){
        ...
        $filename="file1.xls";
            ...
            elseif($this->pdflib=="XLS"){
                 ...
                 include_once dirname(__FILE__)."/ExportXLS.inc.php";
...
public function drawChartFramework($w,$h,$legendpos,$type,$data){
...
public function dbQuery($sql){
    ...
    elseif($this->left($this->cndriver,5)=='adodb')
...
//     echo     print_r( $this->group_count,true)."<br/>";
    ...
    public function display($arraydata,$y_axis=0,$fielddata=false,$maxheight=0)
    {
        ...
        if($arraydata["rotation"]!="")
        {
            ...
            else if($arraydata["rotation"]=="Right")
            {
                $w=$arraydata["width"];
                $arraydata["width"]=$arraydata["height"];
                $arraydata["height"]=$w;
            ...
            else if($arraydata["rotation"]=="UpsideDown")
            {
        ...
        else if($arraydata["type"]=="subreport")
        {
            return $this->runSubReport($arraydata,$y_axis);
        }
        else if($arraydata["type"]=="MultiCell")
        {
        ...
        else if($arraydata["type"]=="RoundedRect")
        {
            if($arraydata['mode']=='Transparent')
                $style='';
            else
            ...
            if($arraydata['printWhenExpression']==""  ||  $this->print_expression($arraydata))
            {
                ...
                $this->pdf->RoundedRect($arraydata["x"]+$this->arrayPageSetting["leftMargin"], $arraydata["y"]+$y_axis, 
                                 $arraydata["width"], $arraydata["height"], $arraydata["radius"], '1111', $style, array(),$arraydata['fillcolor']);
            }
        }
        else if($arraydata["type"]=="Ellipse")
        {
            //$this->pdf->SetLineStyle($arraydata['border']);
        ...
        else if($arraydata["type"]=="Image")
        {
            ...
            else if($this->left($path,22)==  "data:image/jpeg;base64")
            {
                $imgtype="JPEG";
            ...
            else if($this->left($path,22)==  "data:image/png;base64,")
            {
                $imgtype="PNG";
        ...
        else if($arraydata["type"]=="SetTextColor") {
